...
class DependancyTest {
     ...
     public static void ExtractTestFeatures() throws Exception {
      ...
      // Db load from here  
        ...
        ResultSet rs=st.executeQuery("select * from reviewtest order by id asc");
        while(rs.next())
        { 
            int id=rs.getInt("id");
        ...
        // update dependancy rules into table
         String rset= sb.toString();
          Statement stmt=(Statement) conn.createStatement();
             stmt.executeUpdate("update reviewtest set dependancy = '"+rset+"' where id='"+id+"'");
